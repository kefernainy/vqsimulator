...
var firstSegmentArray = [];  //the first group of segments array
var secondSegmentArray = []; // the second group of segments array
...
function distribute(f, segments, COMean, VTMean, lungSegmentArray) {
...
var VTArray = [];
	...
	for (var i = 0; i < segments; i++) {
		randomCO = COMean + Math.round(COMean/2 * f());
		randomVT = VTMean + Math.round(VTMean/2 * f());
...
var getEquilibrium = function(lungSegmentArray) {
...
var calculateTotalSat = function(lungSegmentArray) {
...
console.log("Segment Oxygen Content: ", oxygenContentTotal);
console.log("Segment Oxygen carrying capacity: ", ((totalCO * Hb * 1.34)/100), " totalCO: ", totalCO );
console.log("Segment CO2 Removed: ", CO2ContentRemoved);
	
	return {CO: totalCO, oxygenContent:oxygenContentTotal, Saturation: saturationOfArterialBlood, CO2Removed: CO2ContentRemoved};
	
	
};

var mixedSaturations = function(Array){
	var HbContent = 0;
	var CO2Removed=0;
	var totalCO=0;
	var oxygenContent=0;
Array.forEach(function(e){
	totalCO +=e.CO;
	CO2Removed += e.CO2Removed;
	oxygenContent += e.oxygenContent;
});

var debug = ("Saturation: " + (oxygenContent/((totalCO * Hb * 1.34) / 100)*100).toFixed(0)+ "%<br/> CO2 Removed: "+CO2Removed.toFixed(0));
$("#debug").html(debug);
console.log("Mixed Oxygen Content: "+ oxygenContent);
console.log("Total Carrying Capacity: "+ totalCO*Hb*1.34/100);
console.log("Total CO2 Removed: "+CO2Removed);

return {OxygenSaturation: oxygenContent/((totalCO * Hb * 1.34) / 100), CO2Removed:CO2Removed}
...
$(document).ready(function() {
	...
	var $mainSaturation= $('#mainSaturation');
		var $extraSaturation= $('#extraSaturation');
	...
	$coRangeMain.change(function() {
		/*attempt to limit the CO and VT to be distributed between the segments*/
		
		$coRangeExtra.val(COAVAILABLE-$coRangeMain.val());
		
		
		reset();
		console.log("-----");
		segmentsMain = segmentsAvailable - $segmentsExtra.val();
		// distribute the two groups separately
		// get Equilibrium separately
		// calculate total sats separately
		// calculate cumulative saturation and CO2 removal
		distribute(rnd6, segmentsMain, Number($coRangeMain.val()), Number($vtRangeMain.val()),firstSegmentArray);
		console.log(Number($coRangeMain.val()), " ", Number($coRangeExtra.val()))
		distribute(rnd6, $segmentsExtra.val(), Number($coRangeExtra.val()), Number($vtRangeExtra.val()),secondSegmentArray);
		drawGraphics();
		getEquilibrium(firstSegmentArray);
		this.firstSegmentNumbers = calculateTotalSat(firstSegmentArray);
		getEquilibrium(secondSegmentArray);
		this.secondSegmentNumbers = calculateTotalSat(secondSegmentArray);
		
		
	$extraSaturation.html("");
	$mainSaturation.html("");
		
if (!isNaN(this.secondSegmentNumbers.Saturation*100)){$extraSaturation.html((this.secondSegmentNumbers.Saturation*100).toFixed(0)+"%");}

if (!isNaN(this.firstSegmentNumbers.Saturation*100)){$mainSaturation.html((this.firstSegmentNumbers.Saturation*100).toFixed(0)+"%");}

		updateOutputs();
		mixedSaturations([this.firstSegmentNumbers, this.secondSegmentNumbers]);//getting saturations of the two groups mixing together
	});
	$vtRangeMain.change(function() {
		$vtRangeExtra.val(VTAVAILABLE-$vtRangeMain.val());
		$('#coRangeMain').trigger("change");

	});

	$coRangeExtra.change(function() {
		$coRangeMain.val(COAVAILABLE-$coRangeExtra.val());
		$('#coRangeMain').trigger("change");

	});
	$vtRangeExtra.change(function() {
		$vtRangeMain.val(VTAVAILABLE-$vtRangeExtra.val());
		$('#coRangeMain').trigger("change");
